...
import { ListBestSellerProductsResult } from './list-best-seller-products.result';

@QueryHandler(ListBestSellerProductsQuery)
export class ListBestSellerProductsHandler
  implements
    IQueryHandler<ListBestSellerProductsQuery, ListBestSellerProductsResult>
{
  constructor(
    @InjectPinoLogger(ListBestSellerProductsHandler.name)
  ...
  async execute(
    query: ListBestSellerProductsQuery,
  ): Promise<ListBestSellerProductsResult> {
    ...
    const collection = await this.repository.getBestSellerProducts({
      ...query,
    ...
    return new ListBestSellerProductsResult(collection);
  }
}
