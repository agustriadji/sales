...
import { ListCategoryProductsResult } from './list-category-products.result';

@QueryHandler(ListCategoryProductsQuery)
export class ListCategoryProductsHandler
  implements
    IQueryHandler<ListCategoryProductsQuery, ListCategoryProductsResult>
{
  constructor(
    @InjectPinoLogger(ListCategoryProductsHandler.name)
  ...
  async execute(
    ...
    const collection = await this.repository.getCategoryProducts(query);
    ...
    return new ListCategoryProductsResult(collection);
  }
}
