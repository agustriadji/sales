...
import { ListFlashSaleProductsResult } from './list-flash-sale-products.result';

@QueryHandler(ListFlashSaleProductsQuery)
export class ListFlashSaleProductsHandler
  implements
    IQueryHandler<ListFlashSaleProductsQuery, ListFlashSaleProductsResult>
{
  constructor(
    @InjectPinoLogger(ListFlashSaleProductsHandler.name)
  ...
  async execute(
    query: ListFlashSaleProductsQuery,
  ): Promise<ListFlashSaleProductsResult> {
    ...
    const collection = await this.repository.listFlashSaleProducts(query);

    await this.service.resolvePromotions(query.identity, collection.data, true);
    ...
    return new ListFlashSaleProductsResult(collection);
  }
}
