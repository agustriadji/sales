...
import { ListProductsResult } from './list-products.result';

@QueryHandler(ListProductsQuery)
export class ListProductsHandler
  implements IQueryHandler<ListProductsQuery, ListProductsResult>
{
  MAX_SEARCH = 1000;

  constructor(
    @InjectPinoLogger(ListProductsHandler.name)
  ...
  async execute(query: ListProductsQuery): Promise<ListProductsResult> {
    ...
    const collection = await this.repository.listProducts({
      ...query,
    ...
    return new ListProductsResult(collection);
  }
}
