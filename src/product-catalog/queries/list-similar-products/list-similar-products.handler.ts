...
import { ListSimilarProductsResult } from './list-similar-products.result';

@QueryHandler(ListSimilarProductsQuery)
export class ListSimilarProductsHandler
  implements IQueryHandler<ListSimilarProductsQuery, ListSimilarProductsResult>
{
  constructor(
    @InjectPinoLogger(ListSimilarProductsHandler.name)
  ...
  async execute(
    query: ListSimilarProductsQuery,
  ): Promise<ListSimilarProductsResult> {
    ...
    const collection = await this.repository.getSimilarProducts(query);
    ...
    return new ListSimilarProductsResult(collection);
  }
}
